...
I'll save the printout of the dataframe's head and just go to the visualization, which looks like this (the red vertical line marks the year _Roe_ was decided, 1973), utilizing the following code:
...
(_Nota bene_: It would probably also be nice to update the .csv file/database with opinions that occurred after 2016, bringing them more up-to-date. I'll see if I can write-up something about using the really fantastic [Court Listener](https://www.courtlistener.com/api/bulk-info/) API to fetch the plain text of the opinions from the years between 2016 and 2022.)